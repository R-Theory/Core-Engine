...
export default function IntegrationManager({ className = '' }: IntegrationManagerProps) {
  ...
  const fetchIntegrations = async () => {
    try {
      const response = await fetch('/api/v1/settings/integrations/available')
      if (response.ok) {
        const data = await response.json()
        
        // Transform backend data to frontend format
        const transformedIntegrations = Object.entries(data).map(([key, value]: [string, any]) => ({
          service_name: key,
        ...
        return [
          {
            ...
            required: true,
            description: 'Your institution\'s Canvas API endpoint'
          },
          {
            name: 'access_token',
            ...
            <div key={index} className="glass-card-hover p-6 relative group">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className={`w-3 h-3 rounded-full ${
                  integration.is_connected ? 'bg-emerald-500' : 'bg-slate-400'
                }`} />
              </div>
